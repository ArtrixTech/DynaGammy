...
gcroot<Gammy::MyForm^> f;

HDC screenDC = GetDC(NULL); //GDI Device Context of entire screen
...
{
	HANDLE hStartEvent = CreateEventA(NULL, TRUE, FALSE, name);
...
bool initDXGI() {
	...
	hr = D3D11CreateDevice(
		...
		NULL,                     // Software: we're using a D3D_DRIVER_TYPE_HARDWARE so it's not applicable.
		NULL,                     // Flags: maybe we need to use D3D11_CREATE_DEVICE_BGRA_SUPPORT because desktop duplication is using this.
		NULL,					  // Feature Levels:  what version to use.
...
void updateLabel() {
	f->updateLabels(res, targetRes, lp.t, stop, sleeptime);
}
...
void app() {
	...
	while (true)
	{
		...
		{
			++stop;
			CreateThread(0, 0, (LPTHREAD_START_ROUTINE)adjustBrightness, NULL, NULL, NULL);
...
void checkGammaRange()
{
	...
	s = RegGetValueW(HKEY_LOCAL_MACHINE, subKey, L"GdiICMGammaRange", RRF_RT_REG_DWORD, 0, 0, 0);
	...
	s = RegCreateKeyExW(HKEY_LOCAL_MACHINE, subKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, NULL);
	...
	if (s == ERROR_SUCCESS)
	{
		...
		if (s == ERROR_SUCCESS)
		{
			MessageBoxW(0, L"Gammy has extended the brightness range. Restart to apply the changes.", L"Gammy", 0);
#ifdef _db
...
int main()
{
	...
	if (!initDXGI() || useGDI) 
	{
		...
		f->setUpdateTimeLabel();
	}

	readSettings();

	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)app, 0, 0, 0);
